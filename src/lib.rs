...
fn get_global_trace_context() -> Result<Arc<TraceContext>> {
    ...
    unsafe extern "system" fn event_record_callback(record: *mut EVENT_RECORD) {
        ...
        if event_opcode == EVENT_TRACE_TYPE_LOAD {
            ...
            let Some(proc_ctx) = context.procs.get(&event.ProcessId) else {
                // Ignore dlls for other processes
                return;
            };
            let proc_ctx = proc_ctx.value();
        ...
        let Some(proc_ctx) = context.procs.get(&proc) else {
        ...
        let entry = proc_ctx.stack_counts_hashmap.entry(stack);
        *entry.or_insert(0) += 1;
